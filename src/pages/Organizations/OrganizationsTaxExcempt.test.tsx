...
import OrganizationsTaxExcempt from './OrganizationsTaxExcempt'
...
describe('OrganizationsTaxExcempt', () => {
  const mockOrganizations = [
    {
      id: '1',
      name: 'Tax Excempt Organization 1',
      tin: '123456789',
      category: 'INDIVIDUAL' as const,
      tax_classification: 'EXCEMPT' as const,
    ...
    {
      id: '2',
      name: 'Tax Excempt Organization 2',
      tin: '987654321',
  ...
  it('renders Tax Excempt organizations page with correct title and description', async () => {
    render(<OrganizationsTaxExcempt />)

    expect(screen.getByText('Tax Excempt Organizations')).toBeInTheDocument()
    expect(screen.getByText('Manage and monitor tax excempt organizations')).toBeInTheDocument()
  })

  it('loads and displays EXCEMPT organizations on mount', async () => {
    render(<OrganizationsTaxExcempt />)

    await waitFor(() => {
      expect(OrganizationService.getAllOrganizations).toHaveBeenCalledWith({ tax_classification: 'EXCEMPT' })
    })
  ...
  it('displays error message when loading fails', async () => {
    const errorMessage = 'Failed to load Tax Excempt organizations'
    vi.mocked(OrganizationService.getAllOrganizations).mockRejectedValue(new Error(errorMessage))

    render(<OrganizationsTaxExcempt />)

    await waitFor(() => {
      expect(screen.getByText('Failed to load Tax Excempt organizations. Please try again.')).toBeInTheDocument()
    })
  })
